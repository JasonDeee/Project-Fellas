...
function Javier() {
  ...
  const updateMessages = (newMessage) => {
    ...
    MessageByPass.current = [...MessageByPass.current, newMessage];
  };
    ...
    try {
      ...
      if (resolvedText.isFinal) {
        ...
        if (resolvedText.ResolveStep < 3) {
          ...
          setMessages((prevMessages) => [
            ...prevMessages,
            { type: "received", content: "I cannot complete this task" },
          ]);
  ...
  const handleSendMessage = async () => {
    ...
    const newMessage = { type: "sent", content: inputMessage };
    updateMessages(newMessage);
    setInputMessage("");
